...
class VaultImporter:
    ...
    def _validate_encryption_before_decrypt(self, file_path: str, password: str = None) -> Tuple[bool, str]:
        ...
        logger.info("Encryption validation passed, ready to decrypt")
    ...
    def detect_format_safe(self, file_path: str) -> Optional[str]:

        try:
            return self._detect_format(file_path)
        except (ValueError, UnicodeDecodeError, OSError):
            logger.warning(f"ERR-3: Could not auto-detect format for {file_path}, manual selection required")
            return None
    ...
    def import_vault(
        ...
        try:
            ...
            if format is None:
                ...
                if format is None:
                    raise ValueError(
                        "ERR-3: Could not detect file format automatically. "
                        "Please select the format manually: " +
                        ", ".join(self.SUPPORTED_FORMATS.keys())
                    )
